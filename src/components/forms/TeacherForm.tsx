...
const schema = z.object({
  ...
  sex: z.enum(["male", "female"], { message: "sex is requered!" }),
  img: z.instanceof(File, { message: "Image is requered!" }),
});
...
const TeacherForm = ({ type, data }: { type: "create" | "update"; data?: any }) => {
  ...
  return (
    <form className="flex flex-col gap-8 " onSubmit={onSubmit}>
      ...
      <div className="flex justify-between flex-wrap gap-4">
        <InputField label="Firs Name" name="firstname" defaultValue={data?.firstname} register={register} error={errors?.firstname} />
        ...
        <div className="flex flex-col gap-2 w-full md:w-1/4 justify-center">
          ...
          {errors?.img?.message && <p className="text-xs text-red-500">{errors?.img.message?.toString()}</p>}
        </div>
      </div>
